...
class InfinityDimensionState extends DimensionState {
  ...
  get productionPerSecond() {
    ...
    if (Ra.isRunning) {
      production = production.times(getRaICEffects("IC3"));
    }
    production = production.times(this.multiplier);
    production = overflow(production, BE("e1.80e308"), BE(0.33));
    return production;
  }
  ...
  get multiplier() {
    ...
    if (Replicanti.areUnlocked && Replicanti.amount.gt(1) && Effarig.isRunning) {
      mult = replicantiMult();
    }
...
export const InfinityDimensions = {
  ...
  tick(diff) {
    ...
    for (let tier = 8; tier > subtract; tier--) {
      InfinityDimension(tier).produceDimensions(InfinityDimension(tier - subtract), diff.div(10));
    }
