...
export function antimatterDimensionCommonMultiplier() {
  ...
  if (Ra.isRunning) {
    multiplier = multiplier.dividedBy(getRaICEffects("IC6"));
  }
...
function applyNDPowers(mult, tier) {
  ...
  if (Ra.isRunning) {
    multiplier = multiplier.times(getRaICEffects("IC8"));
    if (player.postC4Tier !== tier) {
      multiplier = multiplier.pow(getRaICEffects("IC4"));
    }
  }
...
class AntimatterDimensionState extends DimensionState {
  ...
  challengeCostBump() {
    if (InfinityChallenge(5).isRunning || Ra.isRunning) this.multiplyIC5Costs();
  ...
  get productionPerSecond() {
    ...
    let production = amount.times(this.multiplier).times(Ra.isRunning
      ? getRaICEffects("IC3")
      : Tickspeed.perSecond
    );
    ...
    production = overflow(production, BE("e1e100"), BE(0.5));
    return production;
  }
}
...
export const AntimatterDimensions = {
  ...
  get buyTenMultiplier() {
    ...
    mult = mult.times(Effarig.per10MultBoost);
  ...
  tick(diff) {
    ...
    if (Teresa.isRunning) maxTierProduced = 1;
