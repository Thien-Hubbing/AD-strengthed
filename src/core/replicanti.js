...
export function getReplicantChance(type) {
  const chancePowers = new Decimal(TimeStudy(213).effectOrDefault(1)).timesEffectOf(RealityUpgrade(23));
  const logChancePowers = Decimal.log10(player.replicanti.chance).times(chancePowers);
  const actualChance = Decimal.pow(player.replicanti.chance, chancePowers).plus(1);
  switch (type) {
    case "ln":
      return (chancePowers.gt(1e6) ? logChancePowers.mul(2.302585092994046) : Decimal.ln(actualChance));
    case "log2":
      return (chancePowers.gt(1e6) ? logChancePowers.mul(3.321928094887362) : Decimal.log2(actualChance));
    case "log10":
      return (chancePowers.gt(1e6) ? logChancePowers : Decimal.log10(actualChance));
    case "none":
      return actualChance;
    default:
...
export function replicantiLoop(diff) {
  ...
  diff = diff.mul(getReplicantChance("log10"));
  ...
  const singleTickAvg = Replicanti.amount.times(getReplicantChance("none"));
  ...
  if (tickCount.gte(100) || (singleTickAvg.gte(10) && tickCount.gte(1))) {
    ...
    let remainingGain = tickCount.times(getReplicantChance("ln")).times(LOG10_E);
    ...
    const binomialTicks = tickCount.sub(batchTicks.div(getReplicantChance("log2")));
    ...
    for (let t = 0; t < binomialTicks.floor().toNumber(); t++) {
      const reproduced = binomialDistribution(Replicanti.amount, getReplicantChance("none"));
...
export const ReplicantiUpgrade = {
  chance: new class ReplicantiChanceUpgrade extends ReplicantiUpgradeState {
    ...
    get cap() {
      // Chance never goes over 100%, unless you're playing this version.
      return TimeStudy(62) ? DC.BEMAX : DC.D1;
    }
  ...
  interval: new class ReplicantiIntervalUpgrade extends ReplicantiUpgradeState {
    ...
    get costIncrease() {
      const fastScale = Decimal.pow(1e5, this.value.div(10000).recip().pow(0.8));
  ...
  galaxies: new class ReplicantiGalaxiesUpgrade extends ReplicantiUpgradeState {
    ...
    bulkPurchaseCalc() {
      ...
      let logBaseIncrease = new Decimal(25);
      let logCostScaling = DC.D5;
      if (Ra.isRunning) {
        logBaseIncrease = new Decimal(1e3);
        logCostScaling = new Decimal(25);
      } else if (EternityChallenge(6).isRunning) {
        logBaseIncrease = DC.D2;
        logCostScaling = DC.D2;
      }
...
export const Replicanti = {
  ...
  get chance() {
    let power = DC.D1;
    power = power.timesEffectOf(TimeStudy(213));
    return ReplicantiUpgrade.chance.value.pow(power);
  },
  galaxies: {
