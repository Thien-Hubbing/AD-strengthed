...
export const breakInfinityUpgrades = {
  totalAMMult: {
    ...
    effect: () => Decimal.pow(player.records.totalAntimatter.max(1).pow(0.0001).add(1), 0.5),
  ...
  currentAMMult: {
    ...
    effect: () => Decimal.pow(Currency.antimatter.value.max(1).pow(0.0001).add(1), 0.5),
    formatEffect: value => formatX(value, 2, 2)
  },
  galaxyBoost: {
    id: "postGalaxy",
    cost: new Decimal(5e11),
    description: () => `All Galaxies are ${formatPercents(0.75)} stronger`,
    effect: 1.75
  },
  infinitiedMult: {
    id: "infinitiedMult",
  ...
  achievementMult: {
    ...
    effect: () => Math.max(Math.pow(Achievements.effectiveCount, 5), 1),
    formatEffect: value => formatX(value, 2, 2)
  },
