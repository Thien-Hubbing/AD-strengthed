...
class RaPetState extends GameMechanicState {
  ...
  get memoryUpgradeCost() {
    return Decimal.pow(2, this.data.memoryUpgrades).mul(DC.E3);
  }

  get chunkUpgradeCost() {
    return Decimal.pow(5, this.data.chunkUpgrades).mul(5000);
  }
...
export function getRaICEffects(IC) {
  const effects = {
    IC3: Decimal.pow(player.galaxies.mul(0.005).plus(1.05), player.totalTickBought),
    IC4: InfinityChallenge(4).effectValue,
    IC6: Currency.matter.value.clampMin(1),
    get IC6MatterGain() {
      return Decimal.pow(1.1, Math.clamp(Date.now() - player.lastUpdate, 1, 21600000));
    },
    IC7: InfinityChallenge(7).effectValue,
    get IC8() {
      const diff = player.records.thisInfinity.realTime.pow(1.2).sub(player.records.thisInfinity.lastBuyTime);
      return Decimal.div(1, 1.1245).pow(Decimal.max(0, diff));
    }
  };
  return effects[IC];
}
...
export const Ra = {
  ...
  get memoryBoostResources() {
    ...
    boostList.push("Thien's Rebalancing Modifications");
