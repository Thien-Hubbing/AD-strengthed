...
export function updateNormalAndInfinityChallenges(diff) {
  if (NormalChallenge(11).isRunning || InfinityChallenge(6).isRunning || Ra.isRunning) {
    if (AntimatterDimension(2).amount.neq(0)) {
      ...
      Currency.matter.multiply(Decimal.pow(cappedBase, (Ra.isRunning
        ? getRaICEffects("IC6MatterGain")
        : diff.div(20))));
    }
  ...
  if (Ra.isRunning) {
    const realDiff = new Decimal(Math.clamp(Date.now() - player.lastUpdate, 1, 21600000));
    player.chall2Pow = Decimal.min(player.chall2Pow.add(realDiff.div(100).div(1800)), 1);
    player.chall3Pow = player.chall3Pow.times(DC.D1_00038.pow(realDiff.pow(2)));
  } else {
  ...
  if (InfinityChallenge(2).isRunning || Ra.isRunning) {
