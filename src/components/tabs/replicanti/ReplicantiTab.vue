...
export default {
  ...
  computed: {
    ...
    replicantiChanceSetup() {
      const powers = new Decimal(TimeStudy(213).effectOrDefault(1)).timesEffectOf(RealityUpgrade(23));
      const display = value => {
        ...
        if (powers.gt(1e6)) string = `Replication Frequency: ${format(powers.mul(value.plus(1).log10()), 2, 2)}/sec`;
        return string;
      };
      return new ReplicantiUpgradeButtonSetup(
        ReplicantiUpgrade.chance,
        value => display(value),
        cost => `${powers.gt(1e6) ? "" : `+${formatPercents(this.nextPercent)} `}Costs: ${format(cost)} IP`
      );
    },
