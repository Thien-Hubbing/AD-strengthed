...
export default {
  ...
  methods: {
    update() {
      ...
      const logGainFactorPerTick = Decimal.divide(getReplicantChance("ln")
        .mul(getGameSpeedupForDisplay().mul(getReplicantChance("log10")).mul(updateRateMs)), getReplicantiInterval());
      ...
      const log10GainFactorPerTickUncapped = Decimal.divide(getReplicantChance("ln")
        .mul(getGameSpeedupForDisplay().mul(getReplicantChance("log10")).mul(updateRateMs)),
      getReplicantiInterval(false)).dividedBy(Math.LN10);
