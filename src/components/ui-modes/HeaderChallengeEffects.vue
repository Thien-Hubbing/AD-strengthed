...
export default {
  ...
  methods: {
    ...
    updateChallengePower() {
      const isC2Running = NormalChallenge(2).isRunning || Ra.isRunning;
      const isC3Running = NormalChallenge(3).isRunning || Ra.isRunning;
      const isIC6Running = InfinityChallenge(6).isRunning || Ra.isRunning;
      const isIC8Running = InfinityChallenge(8).isRunning || Ra.isRunning;
      ...
      if (isChallengePowerVisible) {
        ...
        if (isIC6Running) powerArray.push(`Matter: ${Ra.isRunning ? "All" : "Antimatter"} Dimensions /
          ${format(new Decimal(1).times(Ra.isRunning
            ? getRaICEffects("IC6")
